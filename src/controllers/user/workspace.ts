...
export const addWorkspace = async (
  ...
  try {
    ...
    const numOfFreeWorkspaces = 10;

    const numOfFreeCollaborators = 10;

    if (!title || !type)
      return next(
        new AppError(
          "Title and Type is required !",
          HttpStatusCodes.BAD_REQUEST
        )
      );

    // Check for premium membership and allowed workspaces
    const numOfCurrentWorkspaces = await Workspace.countDocuments({
    ...
    if (numOfCurrentWorkspaces >= numOfFreeWorkspaces && !premiumMembership) {
      return next(
        new AppError(
          "You have reached the limit for free workspaces. Please upgrade to a premium plan to create additional workspaces.",
    ...
    if (emails.length >= numOfFreeCollaborators && !premiumMembership) {
      return next(
        new AppError(
          `You cannot add morethan ${numOfFreeCollaborators} collaborators. Please upgrade to a premium plan to add more collaborators.`,
          HttpStatusCodes.UNAUTHORIZED,
          {
            isPremiumUser: false,
            notify: true,
    ...
    if (emails.length > 0) {
      ...
      new AppResponse(
        res,
        HttpStatusCodes.OK,
        "Workspace created succcessfully",
        {
          workspace: {
            _id: workspaceId,
            title,
...
export const sendInvitations = async (
  ...
  try {
    ...
    if (!emails || !workspaceId) {
      return next(
        new AppError(
          "Please enter email to send invitations!",
          HttpStatusCodes.BAD_REQUEST
        )
      );
    }
    ...
    if (emails.length === 0) {
      return next(
        new AppError(
          "Please provide emails to send invitation!",
          HttpStatusCodes.BAD_REQUEST
        )
      );
    }

    const workspace: any = await Workspace.findById(workspaceId).populate(
      "createdBy"
    );

    if (!workspace) {
      return next(
        new AppError("Workspace not found!", HttpStatusCodes.NOT_FOUND)
      );
    }
    ...
    if (
      workspace.invitations.length >= numOfFreeCollaborators &&
      !premiumMembership
    ) {
      return next(
        new AppError(
          "You have reached the limit for free collaborators. Please upgrade to a premium plan to add more collaborators.",
          HttpStatusCodes.UNAUTHORIZED,
          {
            isPremiumUser: false,
            notify: true,
...
export const invitationAction = async (
  ...
  try {
    ...
    const arrayFilters = [
      { "invitation._id": new mongoose.Types.ObjectId(invitationId) },
    ];
    ...
    if (result.modifiedCount === 0) {
      return next(
        new AppError(
          "Invitation not found or already updated",
...
export const removeCollaborator = async (
  ...
  try {
    ...
    if (!workspaceId || !collaboratorId) {
      return next(
        new AppError(
          "Workspace id and collaborator id are required !",
          HttpStatusCodes.BAD_REQUEST
        )
      );
    }
    ...
    await Workspace.findByIdAndUpdate(workspaceId, {
      $pull: {
        collaborators: new mongoose.Types.ObjectId(collaboratorId as string),
      },
    });
...
export const getInvitationsSend = async (
  req: Request,
  res: Response,
  next: NextFunction
) => {
...
export const deleteWorkspace = async (
  ...
  try {
    ...
    if (!workspaceId) {
      return next(
        new AppError("Invalid workspace ID", HttpStatusCodes.BAD_REQUEST)
      );
    }
