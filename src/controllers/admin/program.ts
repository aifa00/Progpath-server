...
export const getPrograms = async (
  ...
  try {
    ...
    if (filterBy) {
      if (
        !["week", "month", "year", "pending", "accepted", "rejected"].includes(
          filterBy as string
        )
      ) {
        return next(
          new AppError("Invalid filter option", HttpStatusCodes.BAD_REQUEST)
        );
      }
      ...
      if (filterBy === "week") {
        ...
        ["pending", "accepted", "rejected"].includes(filterBy as string)
      ) {
    ...
    if (sort) {
      if (!["title", "date"].includes(sort as string)) {
        return next(
          new AppError("Invalid sort option", HttpStatusCodes.BAD_REQUEST)
        );
      }
    ...
    new AppResponse(
      res,
      HttpStatusCodes.OK,
      "Programs page loaded successfully!",
      {
...
export const updateProgramStatus = async (
  ...
  try {
    ...
    const program: any = await Program.findById(
      programId,
...
export const editProgram = async (
  ...
  try {
    ...
    if (!title || !description || !features || !languages) {
      return next(
        new AppError(
          "Title, description, features and languages are required",
...
export const deleteProgram = async (
  ...
  try {
    ...
    if (!program) {
      return next(
        new AppError("Program not found to delete", HttpStatusCodes.BAD_REQUEST)
      );
    }
